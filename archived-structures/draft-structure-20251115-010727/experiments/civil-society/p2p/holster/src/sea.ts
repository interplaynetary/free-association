...
// Zod schemas for SEA operations
...
export const signedDataSchema = z.object({
  m: z.union([z.string(), z.record(z.any())]),
  s: z.string(),
})
...
const SEA = {
  pair: async (cb?: (pair: KeyPair) => void): Promise<KeyPair> => {
  ...
  encrypt: async (
    data: any,
    pair: {epriv?: string},
    cb?: (enc: EncryptedData | null) => void,
  ...
  verify: async (
    ...
    const signed = typeof data === "string" ? utils.parse(data) : data
  ...
  sign: async (
    ...
    cb?: (signed: SignedData | null) => void,
    ...
    const hash = await utils.sha256(msg)
    const jwkKey = utils.jwk(pair.pub, pair.priv)
    const alg = {name: "ECDSA", namedCurve: "P-256"}
    const sig = await utils.subtle
      .importKey("jwk", jwkKey, alg, false, ["sign"])
      .then(key =>
        utils.subtle.sign(
  ...
  work: async (
    data: any,
    salt?: string | ((pair: {epriv: string}) => void),
    cb?: (pair: {epriv: string}) => void,
  ): Promise<{epriv: string}> => {
    if (typeof salt === "function") {
      cb = salt
      salt = undefined
    }
    if (typeof salt === "undefined") salt = utils.random(9).toString("utf8")
  ...
  secret: async (
    to: {epub?: string},
    from: {epub?: string; epriv?: string},
    cb?: (secret: {epriv: string} | null) => void,
