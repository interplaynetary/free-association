/**
 * Git-Mesh Storage Adapter
 * 
 * Bridges isomorphic-git with Mesh storage:
 * - Git objects stored in Mesh namespaced by user's public key
 * - Commits signed with Mesh identity
 * - Push/pull sync Git objects to/from Mesh network
 */
...
export async function push(repo: string): Promise<string> {
  ...
  console.log(`✓ Pushed ${stored} objects to Mesh`)
...
export async function pull(repo: string, fromPub?: string): Promise<void> {
  ...
  console.log(`✓ Pulled ${fetchedOids.size} objects from Mesh`)
