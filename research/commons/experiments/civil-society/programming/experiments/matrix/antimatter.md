...
Michael Toomim and Greg Little
Invisible College – Braid Project

Antimatter is the first peer-to-peer sequence synchronization algorithm that can prune its history in a network where peers disconnect, reconnect, and merge offline edits. Prior sequence CRDTs work by storing a history of edits forever, so that they can always merge with peers after disconnecting from them for an arbitrary amount of time. It is challenging to know exactly which bits of information need to be stored in order to merge with arbitrary peers after arbitrary times, especially in a peer-to-peer network. The anitmatter algorithm addresses this challenge, and does the hard work of efficiently keeping track of these details.

Antimatter supports arbitrary simultaneous edits, from arbitrary peers, under arbitrary network delays and partitions, and guarantees full CRDT/OT consistency, while pruning unnecessary history within each partitioned subnet, and across subnets once they reconnect. In steady state, it prunes down to zero overhead. This lets you put synchronizing data structures in more parts of your software, without worrying about memory overhead.

Things it does
CRDT / OT guarantees full consistency
Prunes history down to zero after acknowledgements (2 network round trips)
Arbitrary network topology (hub/spoke, mesh)
New peers can be added to the network anywhere at any time
Arbitrary disconnects and reconnects, at arbitrary times
Peers that connect and disconnect on a stable network don't have to send any information at all to other peers.
Prune disconnected subnets while offline
Even if the subnets recursively break up themselves, and then reconnect
Peers do not need to be known
No peer needs to know all the other peers
Thus you can scale a whole cloud in any subnet
It just keeps track of disconnection events
Doesn't need a rename-version operation
Status
The algorithm is fully-functional. You can use the NPM library today. The algorithm is explained below, in literate-programming style.

How it works
Here's a video explanation from an Invisible College Talks Night:


Network Messages extend HTTP → HTTP2P
This extension creates HTTP semantics over P2P networks:


  HTTP Message      HTTP2P Message     Meaning

  GET Request       Subscribe message  "I want this"
  GET Response      Update message     "This is the current version"
  PUT Request       Update message     "This is the current version"
  PUT Response      Ack local          "My connected peers accept/see this version"
                    Ack global         "Everyone accepts/sees this"
                    Welcome            "A new connection happened"
                    Fissure            "A disconnection happened without warning"
These new network messages allow mutation acknowledgements to distribute through a p2p network, while fissures and welcomes allow entire groups of peers to establish a stable shared connection in the face of unexpected network partitions and dropouts. This allows peers to know, with certainty, when all connected peers have acknowledged and moved on from old versions to newer versions.

History prunes by blooping spans of time
When we know which versions aren't needed by a peer anymore, we can determine which ranges of history we can prune away. We prune history by blooping spans of history together. When a peer bloops a span of history, it removes the distinctions between versions within the span, while still allowing the frontier of the bloop to be reconstructed and merged through.

Blooping history is described in Sync9 and Collapsing Time Machines.
...
send: A callback function to be called whenever this antimatter_crdt wants to send a message over a connection registered with subscribe. The sole parameter to this function is a JSONafiable object that hopes to be passed to the receive method on the antimatter_crdt object at the other end of the connection specified in the conn key.
...
You generally do not need to mess with a message object directly, but below are the various message objects you might see, categorized by their type entry. Note that each object also contains a conn entry with the id of the connection the message is sent over.
...
Returns the element at the ith position (0-based) in the sequence_crdt rooted at root_node, when only considering versions which result in true when passed to is_anc.
...
Sets the element at the ith position (0-based) in the sequence_crdt rooted at root_node to the value v, when only considering versions which result in true when passed to is_anc.
