import {z} from "zod"
import SeaArray, {type EncodingType} from "./array.js"

// Zod schemas for buffer operations
...
export type BufferEncodingType = z.infer<typeof bufferEncodingSchema>

// Secure Buffer implementation for SEA.js
// Compatible with NodeJS safe-buffer but with enhanced security
function SafeBuffer(...props: any[]): SeaArray {
  console.warn("new SafeBuffer() is deprecated, please use SafeBuffer.from()")
  return SafeBufferStatic.from(...props)
}

// Input validation helpers
const validateStringInput = (input: string, encoding: string): void => {
...
function isTypedArrayLike(value: unknown): value is TypedArrayLike {
  return (
    value !== null &&
    typeof value === "object" &&
    "buffer" in value &&
...
const processArrayLikeInput = (input: any): Uint8Array => {
  let bytes: Uint8Array

  if (input instanceof ArrayBuffer) {
    validateBufferSize(input.byteLength, "ArrayBuffer processing")
    bytes = new Uint8Array(input)
  } else if (ArrayBuffer.isView(input) && isTypedArrayLike(input)) {
    validateBufferSize(
      input.byteLength,
...
const SafeBufferStatic = {
  from(input: string | ArrayBuffer | ArrayLike<number>, encoding?: BufferEncodingType | string): SeaArray {
  ...
  concat(arr: Array<ArrayLike<number>>): SeaArray {
  ...
  byteLength(string: string, encoding: BufferEncodingType | string = "utf8"): number {
...
Object.assign(SafeBuffer, SafeBufferStatic)
