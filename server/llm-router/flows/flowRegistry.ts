import type { RecognitionAnalysisRequest, CapacityRecommendationRequest, CodeGenerationRequest, DataAnalysisRequest, ChatCompletionRequest } from '../schemas/requestTypes';
...
export const recognitionAnalysisFlow: FlowDefinition = {
  ...
  promptTemplate: (request: RecognitionAnalysisRequest) => {
    const playerNames = request.players.map(p => p.name).join(', ');
...
${request.players.map(p => `${p.name}: ${JSON.stringify(p.recognitions)}`).join('\n')}
...
export const capacityRecommendationFlow: FlowDefinition = {
  ...
  promptTemplate: (request: CapacityRecommendationRequest) => ({
...
${request.player.surplusCapacities.map(c => `- ${c.type}: ${c.quantity} (${c.description})`).join('\n')}

Their mutual contributors:
${request.contributors.map(c => `
${c.name}:
...
export const codeGenerationFlow: FlowDefinition = {
  ...
  promptTemplate: (request: CodeGenerationRequest) => ({
...
export const dataAnalysisFlow: FlowDefinition = {
  ...
  promptTemplate: (request: DataAnalysisRequest) => {
    let analysisPrompt = '';
...
export const chatFlow: FlowDefinition = {
  ...
  promptTemplate: (request: ChatCompletionRequest) => ({
    user: request.messages.map(m => `${m.role}: ${m.content}`).join('\n\n'),
