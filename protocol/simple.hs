...
providerSharesM ci depth z =
  ...
  where
    ...
    depth1Shares p =
      let contribs = nodeContributors (zipperCurrent p)
          validContribs = catMaybes [Map.lookup c ci | c <- Set.toList contribs]
...
exampleForest = (forest, ci)
  where
    ...
    aliceNode = createRootNode "alice" "Alice" "alice" 100 ["bob", "charlie"] Nothing
    bobNode = createRootNode "bob" "Bob" "bob" 100 ["alice", "charlie"] Nothing
    charlieNode = createRootNode "charlie" "Charlie" "charlie" 100 ["alice", "bob"] Nothing
