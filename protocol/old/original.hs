...
-- Specific type for provider shares cache
type ProviderCacheM a = State (Cache (String, Int)) a
...
mutualFulfillment ci a b = withNodeCache (\z -> mutualFulfillmentM ci z b) a

-- Monadic version of mutualFulfillment
mutualFulfillmentM :: Forest -> TreeZipper -> TreeZipper -> CacheM String Float
mutualFulfillmentM ci a b = do
  let aNode = zipperCurrent a
      bNode = zipperCurrent b
      cacheKey = mutualCacheKey (nodeId aNode) (nodeId bNode)
      computeMutual _ = compute ()
  withCacheM cacheKey computeMutual ()
  where
    compute _ =
...
providerSharesCached cache ci provider maxDepth =
  let providerId = nodeId $ zipperCurrent provider
      ...
      computation = providerSharesCachedM ci provider maxDepth
      (shares, updatedCache) = runState computation (sharesCache cache)
   in (shares, cache {sharesCache = updatedCache})

-- Monadic version of providerSharesCached
providerSharesCachedM :: Forest -> TreeZipper -> Int -> ProviderCacheM ShareMap
providerSharesCachedM ci provider maxDepth = do
  let providerId = nodeId $ zipperCurrent provider
      key = (providerId, maxDepth)
      computeShares _ = providerShares ci provider maxDepth
  withCacheM key computeShares ()
