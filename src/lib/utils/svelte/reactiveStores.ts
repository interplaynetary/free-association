 ...
 */
export function createGunStore<T>(path: string | string[], initialValue?: T): GunStore<T> {
	// Create the Gun node
	const nodePath = typeof path === 'string' ? [path] : path;
	const node = new GunNode<T>(nodePath);
...
export function combineStores<A, B, R>(
	...
	const combinedSubscription = storeA.subscription().combine(storeB.subscription(), combineFn);
 ...
 */
export function mapStore<T, R>(store: GunStore<T>, mapFn: (value: T) => R): GunStore<R> {
...
export function filterCollectionStore<T extends CollectionItem>(
	...
	return {
		...
		node: () => store.node(),
		subscription: () =>
			store.subscription().map((items) => items.filter(([_, item]) => filterFn(item)))
	};
}
 ...
 */
export function debounceStore<T>(store: GunStore<T>, delayMs: number): GunStore<T> {
...
export function switchMapStore<T, R>(
	...
	const switchedSubscription = store
		.subscription()
		.switchMap((value) => projectFn(value).subscription());
...
export function withLatestFromStore<A, B, R>(
	...
	const combinedSubscription = store
		.subscription()
		.withLatestFrom(otherStore.subscription(), combineFn);
 ...
 */
export function createDeepStore<T>(path: string | string[], depth: number = 1): GunStore<T> {
	// Create the Gun node
	const nodePath = typeof path === 'string' ? [path] : path;
	const node = new GunNode(nodePath);
...
export function asyncDerivedStore<S extends Readable<any>[], T>(
	...
	function update() {
		...
		const storeValues = stores.map((store) => get(store)) as {
