...
export class ReactiveComponent implements ReactiveComponentApi {
	...
	private async setupRelationship(relationship: ComponentRelationship): Promise<void> {
		...
		try {
			...
			if (relationship.reactive) {
				...
				if (
					relationship.type === ComponentRelationshipType.CHILD ||
	...
	public getDataStore<T>(propertyName: string, defaultValue?: T): Readable<T> {
		return this.graph.getNodePropertyStore<T>(this.basePath, propertyName, defaultValue as T);
	}
			...
			if (Array.isArray(data)) {
				...
				for (const item of data) {
					if (typeof item === 'object' && item.id) {
						await this.graph.updateNodeProperty([...fullPath, item.id], 'value', item);
					}
				}
	...
	public removeRelationship(name: string): void {
		...
		const index = this.config.relationships.findIndex((r) => this.getRelationshipName(r) === name);
 ...
 */
export function withGraphAwareness(graph: ReactiveGraph, options: GraphAwareOptions = {}) {
	return function graphAwareHOC(Component: any) {
		return function createGraphAwareComponent(
			props: SvelteComponentProps & GraphAwareProps
		): EnhancedSvelteComponent {
...
export function graphAware(
	...
	const { graph, path, config } = params;
