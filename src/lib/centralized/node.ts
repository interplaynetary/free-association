import type { Node, Points, TreeZipper, Ctx, NavigationPath } from './types';
...
// Helper function to create Points (mimics Haskell newtype)
export function makePoints(value: number): Points {
	return { value };
}

// Helper function to extract Points value
...
export function addChild(
	...
	return modifyNode((node) => {
		...
		return {
			...node,
			nodeChildren: updatedChildren,
			// Clear cache since structure has changed
			nodeCache: emptyCache()
		};
	}, zipper);
}
