...
import { addChild, makePoints, createRootNode } from './node';
...
export function createCapacityForest(): { forest: Forest; ci: Forest } {
	...
	const charlieWithChild = addChild(
		'charlie_child',
		makePoints(50),
		['alice', 'bob'],
		null,
	...
	const charlieWithShare = addCapacityShare('bob_pie', bobPieShare, charlieWithChild);

	// Build the forest
	forest = addToForest(forest, aliceWithChild);
	forest = addToForest(forest, bobWithShare);
