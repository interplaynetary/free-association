...
<script lang="ts">
    ...
    import { type RecognitionStore, type NodeEntry, type RecNode } from '../stores/rec';
    ...
    export function zoomin(nodeId: string) {
        ...
        const hierarchyNode = hierarchy.descendants().find(d => d.data._key === nodeId);
    ...
    function subscribeToStoreData() {
        ...
        unsubscribeContributors = store.contributorsStore.subscribe(newContributors => {
                    ...
                    d3.select(this)
                        ...
                        .on("click", async (event) => {
                            ...
                            try {
                                ...
                                const unsubscribe = store.childrenStore.subscribe(children => {
    ...
    function findNodeById(nodeId: string): d3.HierarchyRectangularNode<RecNode> | null {
        if (!hierarchy || !nodeId) return null;
        return hierarchy.descendants().find(d => d.data && d.data._key === nodeId) as d3.HierarchyRectangularNode<RecNode> || null;
    }
    ...
    function _zoomout() {
        ...
        if (currentView.data && currentView.data._key) {
            dispatch('nodeFocus', { id: currentView.data._key, data: currentView.data });
        }
    }
