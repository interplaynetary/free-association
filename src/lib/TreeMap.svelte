<script lang="ts">
    ...
    import { calculateFontSize, name as getDisplayName } from '../utils/fontUtils';
    import { usersMap } from '../utils/userUtils';
    import { onMount, onDestroy } from 'svelte';
    import { type RecognitionStore, type NodeEntry, type RecNode, createRec } from '../stores/rec';
    ...
    const GROWTH_RATE = (d: d3.HierarchyRectangularNode<RecNode>) => d.data.points ? d.data.points * 0.05 : 0;
    const GROWTH_TICK = 50;
    const GROWTH_DELAY = 500;
    let isGrowing = false;
    const SHRINK_RATE = (d: d3.HierarchyRectangularNode<RecNode>) => d.data.points ? d.data.points * -0.05 : 0; // Negative growth rate for shrinking
    ...
    export function getCurrentStore(): RecognitionStore {
        return currentStore;
    }
    ...
    function tile(node: d3.HierarchyRectangularNode<RecNode>, x0: number, y0: number, x1: number, y1: number) {
        ...
        */
        ...
        node.children.forEach(child => {
            (child as any).value = child.data && typeof child.data.points === 'number' ? child.data.points : 0;
    ...
    function subscribeToStoreData() {
        ...
        unsubscribeChildren = currentStore.childrenStore.subscribe(newChildrenData => {
        ...
        unsubscribeContributors = currentStore.contributorsStore.subscribe(newContributors => {
    ...
    function updateTreeData() {
        ...
        */
        ...
        try {
        // Create hierarchy
            hierarchy = d3.hierarchy<RecNode>(hierarchyData as RecNode)
                .sum(d => {
            ...
            try {
                ...
                root = treemap(hierarchy as any) as d3.HierarchyRectangularNode<RecNode>;
    ...
    $: if (store) {
        ...
        console.log('TreeMap using current-store with path:', currentStore.path.join('/'));
    ...
    function position(group: d3.Selection<any, any, any, any>, root: d3.HierarchyRectangularNode<RecNode>) {
        ...
        group.selectAll(".contributor-tags-container")
            ...
            .attr("transform", (d: any) => {
                ...
                const textLines = d === root ? 1 : 
                                (d.data && d.data.name) ? d.data.name.split(/(?=[A-Z][^A-Z])/g).length : 1;
    ...
    function render(group: d3.Selection<SVGGElement, unknown, null, undefined>, root: d3.HierarchyRectangularNode<RecNode>) {
        ...
        const node = group
            ...
            .filter(d => {
                const isRoot = d === root;
                const hasPoints = d.data && typeof d.data.points === 'number' && d.data.points > 0;
        ...
        node.append("text")
            ...
            .data(d => {
                ...
                return (d.data && d.data.name) ? d.data.name.split(/(?=[A-Z][^A-Z])/g) : [''];
            })
            .join("tspan")
            .attr("x", 0)
            .attr("dy", (d, i, nodes) => {
            ...
            .on("click", async (event, d) => {
                ...
                const parentNode = d3.select((event.target as any).parentNode).datum() as d3.HierarchyRectangularNode<RecNode>;
        ...
        typeContainer.each(function(d: d3.HierarchyRectangularNode<RecNode>) {
            ...
            if (d.data._key === currentStore?.path[currentStore.path.length - 1]) {
                ...
                tagsArray = Array.isArray(d.data.tags) ? d.data.tags : Object.keys(d.data.tags);
            ...
            const addContributorButton = tagWrapper.append("div")
            ...
            tagsArray.forEach((tagId: string) => {
                ...
                const pillContainer = tagWrapper.append("xhtml:div")
                ...
                tagPillComponent.$on('click', (event) => {
                    ...
                    const currentName = usersMap.has(tagId) ? usersMap.get(tagId)! : tagId;
                ...
                tagPillComponent.$on('remove', (event) => {
                    ...
                    if (d.data._key === currentStore?.path[currentStore.path.length - 1]) {
                        ...
                        currentStore.removeTag(tagId).then(() => {
        ...
        node.filter(d => true)
            ...
            .on("mousedown touchstart", (event, d: d3.HierarchyRectangularNode<RecNode>) => {
                event.preventDefault();
                        ...
                        growthTimeout = setTimeout(() => {
                            ...
                            if (isTouching && activeNode === d) {
                                ...
                                growthInterval = setInterval(() => {
                                    ...
                                    const currentPoints = (d.data && typeof d.data.points === 'number') ? d.data.points : 0;
                                    ...
                                    nodes.select("text")
                                        ...
                                        .style("font-size", d => {
                                            ...
                                            const rectWidth = nd === root ? width : x(nd.x1) - x(nd.x0);
                                            const rectHeight = nd === root ? 50 : y(nd.y1) - y(nd.y0);
                                            return calculateFontSize(nd,root, x, y, currentView) + "px";
                                        });
                                    ...
                                    nodes.select(".contributor-tags-container")
                                        ...
                                        .attr("transform", d => {
                                            ...
                                            const textLines = nd === root ? 1 : 
                                                             (nd.data && nd.data.name) ? nd.data.name.split(/(?=[A-Z][^A-Z])/g).length : 1;
                                            
                                            // Offset from center - move down by half the text height plus padding
                                            const fontSize = calculateFontSize(nd, root, x, y, currentView);
                                            const fontSizeNumber = typeof fontSize === 'number' ? 
                                                fontSize : 12; // Fallback to reasonable default
                                            const verticalOffset = (textLines * 1.2 * fontSizeNumber / 2) + 10;
            ...
            .on("mouseup touchend touchcancel", (event, d) => {
        ...
        node.filter(d => d === root)  // Only for the root navigation rectangle
            .each(function(d) {
                ...
                while (currentNode) {
                    if (currentNode.data === root.data) {  // data is the original root passed to createTreemap
                ...
                if (!isContributorTree) {
                    ...
                    d3.select(this)
                        ...
                        .on("click", async (event) => {
                            ...
                            const currentLevelPoints = currentView.children 
                                ? currentView.children.reduce((sum, node) => sum + (node.data.points || 0), 0)
                            ...
                            try {
                                ...
                                console.log('Adding child node using store with path:', currentStore.path.join('/'));
                                ...
                                const unsubscribe = currentStore.childrenStore.subscribe(children => {
                                    // Check if our new node exists in children
                                    const newNodeExists = children.some(([id]) => id === newNodeId);
                                    ...
                                    if (newNodeExists) {
                                        ...
                                        const newNodeGroup = group.selectAll("g")
                                            .filter((d: any) => d.data && d.data._key === newNodeId);
                                            
                                        if (!newNodeGroup.empty()) {
                                            const nodeData = newNodeGroup.datum() as d3.HierarchyRectangularNode<RecNode>;
                                            ...
                                            const input = container.append("xhtml:input")
                                                .attr("class", "new-node-input")
                                                .style("font-size", calculateFontSize(nodeData, root, x, y, currentView) + "px")
                                                .attr("value", "")
                    ...
                    d3.select(this as Element)
                        ...
                        .on("click", (event) => {
                            ...
                            d3.selectAll(".delete-button text")
                                .attr("fill", () => deleteMode ? "#ff4136" : "#000");
                                
                            // Show a visual indicator or toast message
                            showToastMessage(deleteMode ? "Delete mode activated. Click a node to delete it." : "Delete mode deactivated.", deleteMode ? 'warning' : 'success');
                        })
                        .append("text")
                        .attr("fill", "#000")
                        .attr("font-size", "20px")
    ...
    export function zoomin(childId: string) {
        ...
        const target = hierarchy.descendants().find(d => d.data && d.data._key === childId) as d3.HierarchyRectangularNode<RecNode> || null;
        if (!target) {
        ...
        // Update currentStore to reflect the new view
        currentStore = currentStore.getChild(childId);
        console.log('ALERT: Updated currentStore to child path:', currentStore.path.join('/'));
    ...
    export function zoomout() {
        ...
        currentStore.getParent().then(parentStore => {
            if (parentStore) {
                console.log('ALERT: Updating currentStore to parent path:', parentStore.path.join('/'));
                console.log('ALERT: Parent store:', parentStore);
    ...
    function getAppProperty(property: string, defaultValue: any): any {
        ...
        return (window as any).app[property] !== undefined ? (window as any).app[property] : defaultValue;
    }
    ...
    async function saveNodePoints(node: d3.HierarchyRectangularNode<RecNode>, points: number) {
        if (!node || !node.data) return;
        
        try {
            // Only save if the points actually changed
            const currentStorePoints = currentStore.pointsStore ? get(currentStore.pointsStore) : 0;
    ...
    function get<T>(store: { subscribe: (callback: (value: T) => void) => () => void }): T | undefined {
    ...
    function handleUserSelect(event: CustomEvent<{ id: string, name: string }>) {
        ...
        if (dropdownNodeData) {
            ...
            if (currentStore && currentStore.addContributor) {
                currentStore.addContributor(id).then(() => {
    ...
    function showToastMessage(message: string, type: 'success' | 'warning' = 'success') {
