<script lang="ts">
	...
	const note = $derived(placement ? notes[placement.noteId] : null);
	...
	function handleDragOver(e: DragEvent) {
		...
		if (noteId) {
			...
			isDragOver = true;
			isValidDrop = !hasConflict(room.id, slotIndex, 1, noteId);
		}
	}
	...
	function handleDrop(e: DragEvent) {
		...
		const noteId = e.dataTransfer?.getData('noteId');
		if (noteId) {
			const success = placeNote(noteId, room.id, slotIndex, 1);
			if (!success) {
