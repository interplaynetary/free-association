<script lang="ts">
	...
	// Subscribe to stores
	const board = $derived($userBoard);
	const rooms = $derived($sortedRooms);
	const slots = $derived($timeSlots);
...
<div class="grid-container">
	...
	>
		<div class="grid" style="--room-count: {rooms.length}">
			<!-- Time header row -->
			...
			<!-- Grid rows -->
			{#each slots as timeSlot, slotIndex}
				<div class="time-label">
					{formatTime(timeSlot)}
				</div>
				
				{#each rooms as room}
					{@const placement = getPlacementAt(room.id, slotIndex)}
