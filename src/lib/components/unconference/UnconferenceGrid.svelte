<script lang="ts">
	...
	const timeSlots = $derived(unconferenceState.timeSlots);
	const rooms = $derived(Array.from(unconferenceState.rooms.values()).sort((a, b) => a.order - b.order));
	const placements = $derived(unconferenceState.placements);
	const notes = $derived(unconferenceState.notes);
