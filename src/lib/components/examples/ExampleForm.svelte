...
{#if formSubmitted}
  <div class="success-message">
    ...
    <button onclick={() => (formSubmitted = false)}>Close</button>
  </div>
{:else}
  <Form
    {definition}
  ...
  <button class="force-submit" onclick={forceSubmit}>
    Force Submit Form
  </button>
{/if}
  ...
  <div class="debug-info">
    ...
    {#each Object.entries(formState.fields) as [fieldName, fieldData]}
      <div>
        ...
        {(fieldData as any).validation
          ?.map((v: any) => (v.valid ? "✓" : `✗ ${v.message || "Invalid"}`))
          .join(", ")}
      </div>
    {/each}
  </div>
