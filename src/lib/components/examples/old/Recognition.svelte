<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      unsubscribeChildren = recStore.childrenStore.subscribe(() =>
        updateTreeMapData(),
      );
      unsubscribeContributors = recStore.contributorsStore.subscribe(() =>
        updateTreeMapData(),
      );
      unsubscribeTags = recStore.tagsStore.subscribe(() => updateTreeMapData());
      unsubscribeFulfillment = recStore.fulfillmentStore.subscribe(() =>
        updateTreeMapData(),
      );
  ...
  async function updateTreeMapData() {
    ...
    try {
      ...
      const root = TreeNode.fromRecStore(
        rootData,
        recStore.path[recStore.path.length - 1],
      );
      ...
      contributors.forEach(([contributorId, contributorData]) => {
        ...
        const contributorNode = new TreeNode(
          contributorData.name || contributorId,
        );
      ...
      console.log(
        "Recognition: Added contributors to root:",
        contributors.length,
        contributors,
      );
  ...
  {:else if recStore && treeMapData}
    {#if showDebug}
      <div class="debug-info">
        ...
        <div>
          Fulfillment: {Math.round(treeMapData.calculateFulfillment() * 100)}%
        </div>
      </div>
    {/if}
    <TreeMap
