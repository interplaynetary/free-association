<script lang="ts">
  ...
  let {
    ...
    child: d3.HierarchyRectangularNode<NodeData>;
    addContributor?: (detail: {
      nodeId: string;
      clientX: number;
      clientY: number;
  ...
  // Access the store from child data
  let store = $derived(child.data?.store);

  // Directly access store values using $ syntax
  let name = $derived(store?.nameStore);
  let contributors = $derived(store?.contributorsStore);
  ...
  const contributorIds = $derived(
    $contributors
      ? $contributors.map((entry: [string, any]) => entry[0])
  ...
  $effect(() => {
    if (shouldEdit && !isEditing && child.data?.id) {
