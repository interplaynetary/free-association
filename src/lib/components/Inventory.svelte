<script lang="ts">
	...
	let store = $derived(globalState.recStore);
	...
	function createDistributionChart(depth: number, scale: number = 1): HTMLElement {
		...
		for (let i = 1; i <= 5; i++) {
			...
			(bar as HTMLElement).style.backgroundColor = i <= depth ? getDistributionColor(i) : '#e5e7eb';
	...
	function handleDepthChange(chartElement: HTMLElement, newDepth: number) {
		...
		if (entryId) {
			...
			// Update new capacity
			const rowIndex = parseInt(chartElement.getAttribute('data-row-index') || '0');
...
<div>
	{#if inventoryVisible}
		<div>
			<div>
				<h2 class="mb-4 text-2xl font-bold">Capacity Declarations</h2>
			</div>
								...
								<div class="capacity-input-row" data-capacity-id={entry.id}>
									...
									<div class="inputs-container">
										<div class="input-group">
											<label class="input-label" for={`quantity-${entry.id}`}>Quantity:</label>
											<input
												type="number"
										...
										<div class="input-group">
											<label class="input-label" for={`unit-${entry.id}`}>Units:</label>
											<input
												type="text"
												id={`unit-${entry.id}`}
												class="unit-input"
					...
					<div class="capacity-inputs">
						{#each newCapacityEntries as entry, i (i)}
							<div class="capacity-input-row new-capacity-row">
								...
								<div class="inputs-container">
									<div class="input-group">
										<label class="input-label" for={`new-quantity-${i}`}>Quantity:</label>
										<input
											type="number"
											id={`new-quantity-${i}`}
											class="quantity-input"
									...
									<div class="input-group">
										<label class="input-label" for={`new-unit-${i}`}>Units:</label>
										<input
											type="text"
											id={`new-unit-${i}`}
											class="unit-input"
						...
						<button type="button" class="add-capacity-button" onclick={addCapacityRow}>
							+ Add Another Capacity
						</button>
					</div>

					<div class="form-actions">
						<button type="button" class="update-inventory" onclick={saveInventoryChanges}>
							Save Changes
						</button>
						<button type="button" class="cancel-button" onclick={toggleInventory}> Cancel </button>
					</div>
				</div>
