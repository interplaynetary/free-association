<script lang="ts">
	import type { NeedSlot } from '$lib/schema';
	import { NeedSlotSchema } from '$lib/schema';
	...
	interface Props {
		...
		unit?: string;
		canDelete: boolean;
		onupdate?: (slot: NeedSlot) => void;
		ondelete?: (slotId: string) => void;
	}

	let { slot, needId, unit, canDelete, onupdate, ondelete }: Props = $props();
	...
	// Need-specific state
	let needPriority = $state(slot.priority);
	let needUrgency = $state(slot.urgency);
	let needFlexibility = $state(slot.flexibility);
	let needResourceType = $state(slot.resource_type);
	let needNotes = $state(slot.notes);
	...
	let slotTimeZone = $state(slot.time_zone || undefined);
	...
	let slotCountry = $state(slot.country || undefined);
	let slotOnlineLink = $state(slot.online_link || undefined);
	...
	function handleSlotUpdate() {
		const updatedSlot = {
			...
			resource_type: needResourceType,
			notes: needNotes
		};
		...
		const validationResult = NeedSlotSchema.safeParse(updatedSlot);
