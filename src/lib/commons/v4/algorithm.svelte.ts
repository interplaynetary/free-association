...
export interface AllocationOperatorResult {
	...
	updatedNeedVector: Map<string, MultiDimensionalNeedState>;   // N⃗⃗(t+1)
...
export function updateCommitmentMultiDimensionalDamping(
	...
	return {
		...
		damping_factor: damping.global_damping_factor // backward compatibility
	};
}
...
function buildFilterContext(
	pubKey: string,
	commitment: Commitment,
	mutualRecognitionByType?: MultiDimensionalRecognition,
	typeId?: string
): FilterContext {
	const mutualRecognition = typeId && mutualRecognitionByType
		? mutualRecognitionByType[typeId]
		: undefined;
...
// ═══════════════════════════════════════════════════════════════════
// TYPE-SPECIFIC SLOT COMPATIBILITY (E28')
// ═══════════════════════════════════════════════════════════════════

/**
 * Check if slots are compatible including type matching (E28')
 * Extended: type_match(need_i, avail_j) requirement
 */
function slotsCompatibleWithType(needSlot: NeedSlot, availSlot: AvailabilitySlot): boolean {
	// Type matching (CRITICAL for multi-dimensional)
	if (needSlot.need_type_id !== availSlot.need_type_id) {
		return false; // Different need types cannot match
	}
	
	// Standard slot compatibility (time, location, etc.)
	return slotsCompatible(needSlot, availSlot);
}
...
export function computeMultiDimensionalSlotNativeAllocation(
	...
	for (const [typeId, capacitySlots] of slotsByType.entries()) {
		...
		for (const [recipientPub, amount] of Object.entries(typeResult.recipient_totals)) {
			recipientTotals[recipientPub] = (recipientTotals[recipientPub] || 0) + amount;
...
function computePerTypeSlotAllocation(
	...
	for (const availSlot of capacitySlots) {
		...
		for (const [recipientPub, recipientCommitment] of Object.entries(networkCommitments)) {
			...
			// Only consider need slots of matching type
			const matchingNeedSlots = recipientCommitment.need_slots.filter(needSlot => {
				return needSlot.need_type_id === typeId && slotsCompatibleWithType(needSlot, availSlot);
			});
			...
			if (matchingNeedSlots.length > 0) {
				...
				const providerContext = buildFilterContext(
					providerPubKey,
					{ ...providerCommitment, capacity_slots: [availSlot] },
					undefined,
					typeId
				);
		...
		if (totalMutualRecognition > 0) {
			for (const [recipientPub, needSlots] of mutualRecipients.entries()) {
				...
				const dampingFactor = recipientCommitment.multi_dimensional_damping?.damping_factors[typeId] 
					|| recipientCommitment.damping_factor 
					|| 1.0;
