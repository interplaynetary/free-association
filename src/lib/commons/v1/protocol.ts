/**
 * Free Association Protocol - Commons Edition
 *
 ...
 *
 * ARCHITECTURE:
 * - All schemas imported from ./schemas.ts (unified schema source)
 * - Tree operations for recognition/priority calculations
 * - Slot-native two-tier allocation for resource matching
 * - Filter utilities for capacity/need constraints
 */

// All types now from commons/schemas.ts
import type {
	Node,
	RootNode,
	NonRootNode,
 ...
 * from commons/schemas.ts. These complement the legacy tree-based functions
 * and prepare for full migration to slot-native two-tier allocation.
 * ═══════════════════════════════════════════════════════════════════════
 */

/**
 * Extract resource metadata from a capacity or slot
 * Works with both legacy and advanced schema types
 */
export function extractResourceMetadata(
	resource: BaseCapacity | AvailabilitySlot | BaseNeed | NeedSlot | any
): ResourceMetadata {
	return {
		name: resource.name || '',
...
/**
 * Check if a slot has a filter rule (either slot-specific or inherited)
 */
export function hasFilterRule(
	slot: AvailabilitySlot | NeedSlot,
	parentCapacity?: BaseCapacity | BaseNeed
): boolean {
	// Check slot-specific filter first (takes precedence)
	if (slot.filter_rule !== undefined && slot.filter_rule !== null) {
		return true;
	}
	// Check parent capacity filter
	if (parentCapacity?.filter_rule !== undefined && parentCapacity?.filter_rule !== null) {
		return true;
	}
	return false;
}
...
/**
 * Get effective filter rule for a slot (slot-specific overrides parent)
 */
export function getEffectiveFilterRule(
	slot: AvailabilitySlot | NeedSlot,
	parentCapacity?: BaseCapacity | BaseNeed
): any | null {
	// Slot filter takes precedence
	if (slot.filter_rule !== undefined && slot.filter_rule !== null) {
		return slot.filter_rule;
	}
	// Fall back to parent capacity filter
	if (parentCapacity?.filter_rule !== undefined && parentCapacity?.filter_rule !== null) {
		return parentCapacity.filter_rule;
	}
	return null;
}
 ...
 * Used by the two-tier allocation algorithm for time-based matching
 */
export function areSlotsTimeCompatible(
	availabilitySlot: AvailabilitySlot,
	needSlot: NeedSlot
): boolean {
	// If either slot doesn't specify time, assume compatible
	if (!availabilitySlot.start_date || !needSlot.start_date) {
		return true;
	}

	const availStart = new Date(availabilitySlot.start_date);
	const needStart = new Date(needSlot.start_date);

	// Check if dates overlap (simplified - could be more sophisticated)
	if (availabilitySlot.end_date && needSlot.end_date) {
		const availEnd = new Date(availabilitySlot.end_date);
		const needEnd = new Date(needSlot.end_date);

		// Check for any overlap
		return availStart <= needEnd && needStart <= availEnd;
	}

	// If no end dates, check if start dates are the same day
	return availStart.toDateString() === needStart.toDateString();
}

/**
 * Check if two slots have compatible location constraints
 * Used by the two-tier allocation algorithm for location-based matching
 */
export function areSlotsLocationCompatible(
	availabilitySlot: AvailabilitySlot,
	needSlot: NeedSlot
): boolean {
	// If both are online, they're compatible
	if (availabilitySlot.location_type === 'Online' && needSlot.location_type === 'Online') {
		return true;
	}

	// If one is online and the other isn't specified, assume compatible
	if (availabilitySlot.location_type === 'Online' && !needSlot.location_type) {
		return true;
	}
	if (needSlot.location_type === 'Online' && !availabilitySlot.location_type) {
		return true;
	}

	// If both have specific locations, check distance (simplified)
	if (
		availabilitySlot.latitude &&
		availabilitySlot.longitude &&
		needSlot.latitude &&
		needSlot.longitude
	) {
		// Simple distance check - could be made more sophisticated
		const latDiff = Math.abs(availabilitySlot.latitude - needSlot.latitude);
		const lonDiff = Math.abs(availabilitySlot.longitude - needSlot.longitude);

		// Within ~0.1 degrees (roughly 10km) - make this configurable
 ...
 * Helper for building Commitment objects for the two-tier allocation system
 */
...
/**
 * Re-export advanced schema types for convenience
 */
export type {
	Node,
	RootNode,
