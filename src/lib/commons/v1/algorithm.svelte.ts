...
/**
 * Build filter context for a participant
 * 
 * @param pubKey - Participant's public key
 * @param commitment - Participant's commitment
 * @param mutualRecognition - Mutual recognition value (if applicable)
 * @returns FilterContext for filter evaluation
 */
function buildFilterContext(
	...
	return {
		...
		commitment,
		mutualRecognition,
		attributes: (commitment as any).attributes || {}  // Extract attributes if available
	};
}
...
export function computeAllocation(
	...
	// Build need slot buckets using consolidated bucketing functions
	for (const [recipientPub, recipientCommitment] of Object.entries(networkCommitments)) {
		...
		// Note: Filters are now checked bilaterally at the slot level
		// during detailed compatibility checking (see compatibilityMatrix section)
	...
	for (const availSlot of providerCommitment.capacity_slots) {
		...
		// Add from time bucket using consolidated function
		...
		// Filter by location bucket (intersection) using consolidated function
		...
		for (const item of candidateSlots) {
			// Check time/location compatibility
			if (!slotsCompatible(item.needSlot, availSlot)) continue;
			
			// BILATERAL FILTER CHECK
			// Build filter contexts for provider and recipient
			...
			// Check if both filters pass (bilateral consent)
			if (!passesSlotFilters(item.needSlot, availSlot, providerContext, recipientContext)) {
				continue; // Filter rejected - skip this slot pairing
			}
			
			// Compatible! Add to matrix
