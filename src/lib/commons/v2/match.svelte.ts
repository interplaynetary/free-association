 ...
 * 
 * Note: This function only checks time/location compatibility.
 * Filter checking requires provider/recipient context and should be done
 * in the allocation algorithm (see algorithm.svelte.ts)
 * 
 ...
 * 
 * SPACE-TIME AWARE MATCHING:
 * - Only matches slots that are compatible in both time AND location
 * - Each need slot can match multiple availability slots (if compatible)
 * - Each availability slot can match multiple need slots (if compatible)
 * - Slots at the SAME time/location from the same person are naturally handled
 *   separately by the slot-native algorithm (each slot allocates independently)
 * 
 * IMPORTANT: This function respects that a need at "Monday 9am in SF" CANNOT
 * be fulfilled by capacity at "Tuesday 9am in SF" or "Monday 9am in NYC".
 * Space-time must align for allocation to occur.
 * 
...
export function matchNeedToCapacitySlots(
	...
	// Each pairing is checked for space-time compatibility
	for (const needSlot of need.need_slots) {
		for (const availSlot of capacity.availability_slots) {
			...
			// This ensures space-time alignment
 ...
 * For allocation purposes, use space-time aware matching instead.
 ...
 * 
 * NOTE: This is a simple total, not space-time aware.
 * Actual fulfillment must consider slot-level compatibility.
 */
 ...
 * 
 * IMPORTANT: This respects space-time constraints. Need slots at different
 * times/locations are treated separately and cannot be aggregated.
 * 
...
export function calculateSlotCompatibleAmount(
	...
	// Match slots respecting space-time compatibility
...
/**
 * Calculate space-time aware need amount
 * Groups need slots by space-time signature and returns unique space-time combinations
 * 
 * This is the "true" need considering that slots at the same time/location
 * should be aggregated, but slots at different times/locations are separate.
 * 
 * @param need - Need with slots
 * @returns Array of space-time combinations with aggregated quantities
 */
export function getSpaceTimeNeedProfile(need: BaseNeed): Array<{
	signature: string;
	quantity: number;
	slots: NeedSlot[];
}> {
	const groups = groupSlotsBySpaceTime(need.need_slots);
...
/**
 * Calculate space-time aware capacity amount
 * Groups availability slots by space-time signature
 * 
 * @param capacity - Capacity with slots
 * @returns Array of space-time combinations with aggregated quantities
 */
export function getSpaceTimeCapacityProfile(capacity: BaseCapacity): Array<{
	signature: string;
	quantity: number;
	slots: AvailabilitySlot[];
}> {
	const groups = groupSlotsBySpaceTime(capacity.availability_slots);
	return Array.from(groups.entries()).map(([signature, data]) => ({
		signature,
		quantity: data.quantity,
		slots: data.slots
