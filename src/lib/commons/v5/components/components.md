...
**User Flow:**
1. User lands on app
2. Sees login/signup options
3. Authenticates via Holster
4. System initializes stores

**Components:**
```
components/auth/
├── AuthPage.svelte           # Main auth page
│   ├── LoginForm.svelte      # Email/password login
│   └── SignupForm.svelte     # New user registration
└── AuthGuard.svelte          # Wrapper that ensures auth
```

**Schemas Used:** None (Holster handles this)

---

### 🌳 **Phase 2: Recognition Tree Builder**

**User Flow:**
1. User creates root node ("My Values")
2. Adds child nodes (e.g., "Healthcare", "Food", "Education")
3. Assigns points to each child
4. Adds contributors to nodes (with weighted points)
5. System auto-computes recognition weights
6. User sees computed weights in real-time

**Components:**
```
components/recognition/
├── RecognitionTreeEditor.svelte     # Main tree editor
│   ├── TreeVisualization.svelte     # D3/visual tree display
│   ├── NodeEditor.svelte            # Edit single node
│   │   ├── NodeForm.svelte          # Name, points, manual fulfillment
│   │   ├── ContributorsList.svelte  # List of contributors
│   │   └── ContributorForm.svelte   # Add/edit contributor (id + points)
│   ├── NodeCard.svelte              # Display node info
│   └── AddNodeButton.svelte         # Add child/sibling
│
├── RecognitionWeightsView.svelte    # Computed weights display
│   ├── WeightBar.svelte             # Visual bar for each person
│   └── WeightsList.svelte           # Table view
│
└── PubKeyInput.svelte               # Search/select other users
```
...
**Stores:**
- `myRecognitionTreeStore` (write)
- `myRecognitionWeights` (read - derived)
...
**Phase 1 (MVP):**
1. ✅ Auth (simple, using Holster)
2. ✅ Recognition Tree Builder (core feature)
