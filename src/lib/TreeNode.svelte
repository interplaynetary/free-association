<script lang="ts">
    ...
    function addTagsContainer(selection: d3.Selection<HTMLElement, unknown, null, undefined>, rectWidth: number, rectHeight: number) {
        ...
        if (node) {
            node.getContributors().forEach(contributor => {
                const pillContainer = wrapper.append("xhtml:div")
    ...
    function handleNodeClick(event: MouseEvent) {
        ...
        if (deleteMode && !isRoot && store && parentId && node?.id) {
            ...
            if (confirm(`Delete "${node.name || 'Unnamed'}"?`)) {
                ...
                store.removeChild(node.id).then(() => {
                    // Send custom event
                    if (domElement) {
