<script lang="ts">
  ...
  import { RelationalForm, type RelationalFormDefinition, type RelationalField, type ValidationResult } from '../utils/basic/RelationalForm';
  ...
  {#if formState}
    {#if submissionSuccessful && preserveStateAfterSubmit}
      <div class="success-message">
        ...
        <button class="close-button" on:click={() => submissionSuccessful = false}>Close</button>
      </div>
    {/if}
    
    <form on:submit|preventDefault={handleSubmit} class:valid={formState.formValid} class:submitting={formState.submitting} class:success={submissionSuccessful}>
      {#if definition.title}
                ...
                {#if formState.suggestionsVisible[field.name] && formState.fields[field.name].suggestions.length > 0}
                  <ul class="suggestions">
                    {#each formState.fields[field.name].suggestions as suggestion}
                      <li on:click={() => handleSelectSuggestion(field.name, suggestion)} aria-label={field.displayProperty ? suggestion[field.displayProperty] : suggestion}>
                        {field.displayProperty ? suggestion[field.displayProperty] : suggestion}
                      </li>
                    {/each}
                  </ul>
                {/if}
