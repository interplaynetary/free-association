<script lang="ts">
  ...
  let activePanel = $state<"none" | "inventory" | "login" | "charts">("none");

  // Dynamic component registry
  const panelComponents = $state({
    inventory: {
      component: null as any,
      loader: async () => import("./Inventory.svelte"),
      props: () => ({ store }),
    },
    login: {
      component: null as any,
      loader: async () => import("./LogIn.svelte"),
      props: () => ({
    ...
    charts: {
      component: null as any,
      loader: async () => import("./stackedBar.svelte"),
      props: () => ({
        data: [
  ...
  const PanelComponent = $derived(
    activePanel !== "none" ? panelComponents[activePanel].component : null,
  );

  // Panel props
  const panelProps = $derived(
    activePanel !== "none" ? panelComponents[activePanel].props() : {},
  );

  // Set the active panel
  function setActivePanel(panel: "none" | "inventory" | "login" | "charts") {
    // Only load component if it's a different panel
    if (panel !== "none" && panel !== activePanel) {
  ...
  // Panel handler functions for different button clicks
  function handleInventoryClick() {
    setActivePanel(activePanel === "inventory" ? "none" : "inventory");
    console.log("Inventory panel toggled:", activePanel === "inventory");
  }

  function handlePeerClick() {
    setActivePanel(activePanel === "login" ? "none" : "login");
    console.log("Login panel toggled:", activePanel === "login");
  }

  function handleChartsClick() {
    setActivePanel(activePanel === "charts" ? "none" : "charts");
    console.log("Charts panel toggled:", activePanel === "charts");
  }
  ...
  <div class="node-container" bind:this={nodeContainer}>
    ...
    <div class="node-header">
      <Header
        ...
        {store}
        on:navigateToPath={(e: CustomEvent) => navigateToPathIndex(e.detail)}
      />
    </div>

    {#if activePanel !== "none"}
      <!-- Show active panel component -->
      <div class="{activePanel}-wrapper panel-wrapper">
        {#if PanelComponent}
          <PanelComponent {...panelProps} />
        {:else}
          <div class="loading">Loading {activePanel} panel...</div>
        {/if}
      </div>
    {:else}
          ...
          {#each hierarchyData.children || [] as child (child.data?._key)}
            <div
              ...
              class:growing={isGrowing &&
                activeGrowthNodeId === child.data?._key}
...
<style>
  ...
  .inventory-wrapper {
    /* Existing inventory styles remain */
  }
