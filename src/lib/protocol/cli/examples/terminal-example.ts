...
function runAliceIteration() {
	...
	for (const allocation of result.allocations) {
		...
		console.log(`  → ${recipient}: ${allocation.quantity.toFixed(2)} ${allocation.type_id} (${allocation.tier})`);
	}
	...
	while (iteration < maxIterations) {
		...
		for (const [pubKey, commitment] of Object.entries({
			...
			const updatedNeedSlots = (commitment.need_slots || []).map(slot => {
				const received = allAllocations[pubKey][slot.type_id] || 0;
				return {
					...slot,
			...
			for (const { provider, result } of allResults) {
				if (result.allocations.length > 0) {
					...
					for (const allocation of result.allocations) {
						...
						console.log(`    → ${recipientName}: ${allocation.quantity.toFixed(2)} ${allocation.type_id} (${allocation.tier})`);
					}
				}
			}
		}
...
function runMultipleIterations(maxIterations: number = 5) {
	...
	for (let i = 0; i < maxIterations; i++) {
		...
		for (const allocation of result.allocations) {
			receivedByType[allocation.type_id] =
				(receivedByType[allocation.type_id] || 0) + allocation.quantity;
		}
