 ...
 * 
 * NEW (v6): Uses H3 spatial indexing when available for efficient radius queries
 * LEGACY: Falls back to city/country/coordinate matching for backward compatibility
 * 
...
export function locationsCompatible(
	...
	// **NEW: H3-based spatial matching (when available)**
	if (slot1.h3_index && slot2.h3_index) {
		// Use the larger search radius of the two slots
	...
	// If we have precise coordinates for both, check proximity
	// Use search_radius_km if available, otherwise default to 50km
	if (
