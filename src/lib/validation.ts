...
/**
 * Parse and validate collective capacities data
 * @param collectiveCapacitiesData Raw collective capacities data
 * @returns Validated collective capacities collection or empty object if validation fails
 */
export function parseCollectiveCapacities(collectiveCapacitiesData: unknown) {
	return parseData(collectiveCapacitiesData, {
		schema: z.record(z.string(), CollectiveCapacitySchema),
		defaultValue: {},
		functionName: 'collective capacities',
		enableLogging: true,
		preprocess: (data: unknown) => {
			if (!data || typeof data !== 'object') {
				return data;
			...
			// If it's a collective capacities collection, migrate each capacity
			const capacitiesObj = data as Record<string, any>;
			const migratedCapacities: Record<string, any> = {};
