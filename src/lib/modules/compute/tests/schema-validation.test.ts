 ...
 * 
 * Coverage:
 * - Primitive schema validation (Identifier, MeshPath, PubKey, etc.)
	...
	describe('MeshPathSchema', () => {
		it('accepts valid paths', () => {
			...
			for (const path of valid) {
				const result = MeshPathSchema.safeParse(path);
				expect(result.success).toBe(true);
			}
		});
...
describe('RDL Schemas - Variable Bindings', () => {
	...
	describe('subscription binding', () => {
		...
		it('requires mesh_path and schema_type', () => {
			const invalid = {
				type: 'subscription'
		...
		it('validates mesh_path format', () => {
			const invalid = {
				type: 'subscription',
				mesh_path: 'invalid path!',
				schema_type: 'Any'
			};
...
describe('RDL Schemas - Computation', () => {
	it('accepts valid computation', () => {
		const computation = {
			...
			compute_fn: 'allocate',
			outputs: {
				result: { type: 'mesh', mesh_path: 'results/allocation' }
			},
			debounce_ms: 200,
			enabled: true,
	...
	it('accepts valid complete program', () => {
		const program = {
			...
			computations: [{
				...
				outputs: {
					state: { type: 'mesh', mesh_path: 'allocation/state' }
				}
			}],
