...
// Mock mesh - inline to avoid hoisting issues
vi.mock('$lib/network/mesh.svelte', () => ({
	meshUser: {
		is: { pub: '0'.repeat(64) },
		get: vi.fn(() => ({
	...
	meshUserPub: { subscribe: vi.fn(), set: vi.fn(), update: vi.fn() }
}));

// Mock mesh utils - inline to avoid hoisting issues
vi.mock('$lib/utils/meshData', () => ({
...
beforeEach(async () => {
	...
	const meshModule = await import('$lib/network/mesh.svelte');
	const meshDataModule = await import('$lib/utils/data/meshData');

	mockMeshUser = meshModule.meshUser;
	mockWriteAtPath = vi.mocked(meshDataModule.writeAtPath);
	mockReadAtPath = vi.mocked(meshDataModule.readAtPath);
	mockListenAtPath = vi.mocked(meshDataModule.listenAtPath);
