...
describe('Path Manipulation', () => {
	...
	describe('prefixMeshPath', () => {
		it('should prefix a simple path', () => {
			const result = prefixMeshPath(testHash, 'tree');

			expect(result).toBe('abc123def456/tree');
		});

		it('should prefix a nested path', () => {
			const result = prefixMeshPath(testHash, 'nodes/node1/data');
		...
		it('should handle empty path', () => {
			const result = prefixMeshPath(testHash, '');

			expect(result).toBe('abc123def456/');
		});
	});

	describe('unprefixMeshPath', () => {
		it('should unprefix a simple path', () => {
			const result = unprefixMeshPath('abc123def456/tree');

			expect(result).toBe('tree');
		});

		it('should unprefix a nested path', () => {
			const result = unprefixMeshPath('abc123def456/nodes/node1/data');

			expect(result).toBe('nodes/node1/data');
		});

		it('should return original path if no prefix', () => {
			const result = unprefixMeshPath('tree');

			expect(result).toBe('tree');
		});

		it('should handle paths with multiple segments', () => {
			const result = unprefixMeshPath('hash/a/b/c/d');
	...
	describe('Path round-trip', () => {
		it('should maintain path integrity through prefix/unprefix', () => {
			...
			const prefixed = prefixMeshPath(hash, originalPath);
			const unprefixed = unprefixMeshPath(prefixed);
		...
		it('should maintain hash through extract after prefix', () => {
			...
			const prefixed = prefixMeshPath(hash, path);
...
describe('Edge Cases', () => {
	...
	describe('Very long paths', () => {
		it('should handle very long nested paths', () => {
			...
			const prefixed = prefixMeshPath(hash, longPath);
			const unprefixed = unprefixMeshPath(prefixed);
...
describe('Integration', () => {
	...
	it('should support full workflow: register, retrieve, use hash', () => {
		...
		// Use hash for path
		const path = prefixMeshPath(hash, 'tree');
	...
	it('should maintain data isolation with different hashes', () => {
		...
		const path1 = prefixMeshPath(hash1, 'data');
		const path2 = prefixMeshPath(hash2, 'data');
