...
describe('Hybrid Storage - Path Structure', () => {
	...
	test('version path format', () => {
		...
		const versionPath = `${prefixMeshPath(programHash, meshPath)}/_versions/${provSig}`;
	...
	test('latest index path format', () => {
		...
		const latestIndexPath = `${programHash}/_index/latest/${meshPath}`;
...
describe('Hybrid Storage - Path Resolution', () => {
	...
	test('resolve canonical from RDL mesh_path', () => {
		const programHash = 'def456';
		const rdlPath = 'analytics/user_count';

		const canonical = prefixMeshPath(programHash, rdlPath);
	...
	test('extract mesh_path from canonical', () => {
		const canonicalPath = 'abc123/analytics/count';

		// Mesh path is everything after first '/'
		const meshPath = canonicalPath.substring(canonicalPath.indexOf('/') + 1);

		expect(meshPath).toBe('analytics/count');
	});
});
...
describe('Hybrid Storage - Query Patterns', () => {
	...
	test('query latest pattern', () => {
		// Pattern: meshUser.get(canonicalPath).once()
		const programHash = 'abc123';
		const meshPath = 'math/sum';

		const queryPath = prefixMeshPath(programHash, meshPath);
	...
	test('query all versions pattern', () => {
		// Pattern: meshUser.get(`${canonicalPath}/_versions`).map().once()
	...
	test('query specific version pattern', () => {
		// Pattern: meshUser.get(`${canonicalPath}/_versions/${provSig}`).once()
		const canonicalPath = 'abc123/math/sum';
	...
	test('query computation outputs pattern', () => {
		// Pattern: meshUser.get(`${programHash}/_index/computations/${compId}`).map().once()
		const programHash = 'abc123';
	...
	test('query lineage pattern', () => {
		// Pattern: meshUser.get(`${programHash}/_index/lineage/${provId}`).once()
		const programHash = 'abc123';
...
describe('Hybrid Storage - Cross-User Queries', () => {
	...
	test('cross-user canonical path', () => {
		...
		const crossUserPath: [string, string] = [
			pubkey,
			prefixMeshPath(programHash, meshPath)
		];
