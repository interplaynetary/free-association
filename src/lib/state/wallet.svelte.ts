...
const DEFAULT_ENDPOINT = 'wss://westend-rpc.polkadot.io';
...
export async function connectWallet(): Promise<void> {
	...
	try {
		...
		const provider = new WsProvider(DEFAULT_ENDPOINT);
		...
		if (accounts[0]) {
			try {
				...
				balance = `${(free.toNumber() / Math.pow(10, decimals)).toFixed(4)} ${symbol}`;
			} catch (err) {
...
export async function selectAccount(address: string): Promise<void> {
	...
	if (state.api) {
		try {
			...
			const decimals = state.api.registry.chainDecimals[0] || 10;
...
export async function getTokenDecimals(): Promise<number> {
	...
	return state.api.registry.chainDecimals[0] || 10;
}
