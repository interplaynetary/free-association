...
import { user, mesh, inviteCodes } from "./core"
import { newCode } from "./utils"
import type { AccountData } from "$lib/server/schemas/mesh"
...
export async function createInviteCodes(
  ...
  const secret = await mesh.SEA.secret(account, user.is)
  for (let i = 0; i < newCodes.length; i++) {
    const invite = { code: newCodes[i], owner: owner }
    const enc = await mesh.SEA.encrypt(invite, user.is)
    ...
    const shared = await mesh.SEA.encrypt(newCodes[i], secret)
