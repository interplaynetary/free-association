<script lang="ts">
  ...
  async function updateTheme(newTheme: string) {
    try {
      ...
      if (map) {
        ...
        if (layers.length > 0) {
          console.log("First 5 layers:", layers.slice(0, 5).map(l => l.id));
        }
  ...
  export function fitBounds(bounds: maplibregl.LngLatBoundsLike, options: maplibregl.FitBoundsOptions = {}) {
    if (map) {
  ...
  function toggleMapLayers(layerType: 'labels' | 'roads' | 'buildings' | 'water' | 'landmarks' | 'landuse', visible: boolean) {
    const layerPatterns = {
    ...
    switch (layerType) {
      case 'labels': showLabels = visible; break;
      case 'roads': showRoads = visible; break;
      case 'buildings': showBuildings = visible; break;
      case 'water': showWater = visible; break;
      case 'landmarks': showLandmarks = visible; break;
      case 'landuse': showLanduse = visible; break;
    }
    ...
    try {
      ...
      for (const layerId of specificLayers[layerType]) {
        try {
          if (map.getLayer(layerId)) {
            console.log(`Setting known ${layerType} layer ${layerId} to ${visible ? 'visible' : 'none'}`);
      ...
      for (const layer of style.layers) {
        if (typeof layer.id === 'string' && patterns.some(pattern => layer.id.includes(pattern))) {
          try {
            console.log(`Setting ${layerType} layer ${layer.id} to ${visible ? 'visible' : 'none'}`);
...
<div class="map-wrapper" style="height: {height};">
    ...
    <div class="search-container">
      <div class="search-input-container">
        <input
          ...
          class="search-input"
        />
        <button class="search-button" on:click={handleSearch} disabled={isSearching}>
          {#if isSearching}
      ...
      {#if showSearchResults && searchResults.length > 0}
        <div class="search-results">
          {#each searchResults as result (result.id)}
            <div class="search-result-item" on:click={() => goToSearchResult(result)} title={result.name} aria-label={result.name}>
  ...
  {#if showStyleControls}
    <div class="map-controls">
      ...
      <button class="control-button" class:active={showBasemapPanel} on:click={toggleBasemapPanel} title="Basemap" aria-label="Basemap">
      ...
      <!-- Layers Button -->
      <button class="control-button" class:active={showLayersPanel} on:click={toggleLayersPanel} title="Layers" aria-label="Layers">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    {#if showBasemapPanel}
      <div class="side-panel basemap-panel">
        <div class="panel-header">
          <h3>Basemap</h3>
          <button class="close-button" on:click={() => showBasemapPanel = false}>×</button>
        </div>
    ...
    {#if showLayersPanel}
      <div class="side-panel layers-panel">
        <div class="panel-header">
          <h3>Layers</h3>
          <button class="close-button" on:click={() => showLayersPanel = false}>×</button>
        </div>
        <div class="panel-content">
          ...
          <div class="layer-list">
            ...
            <div class="layer-item">
              <label class="layer-toggle">
                <input type="checkbox" bind:checked={showLabels} on:change={() => toggleLabels(showLabels)}>
                <span class="layer-name">Labels</span>
              </label>
            </div>
            <div class="layer-item">
              <label class="layer-toggle">
                <input type="checkbox" bind:checked={showRoads} on:change={() => toggleRoads(showRoads)}>
                <span class="layer-name">Roads</span>
              </label>
            </div>
            <div class="layer-item">
              <label class="layer-toggle">
                <input type="checkbox" bind:checked={showBuildings} on:change={() => toggleBuildings(showBuildings)}>
                <span class="layer-name">Buildings</span>
              </label>
            </div>
            <div class="layer-item">
              <label class="layer-toggle">
                <input type="checkbox" bind:checked={showWater} on:change={() => toggleWater(showWater)}>
                <span class="layer-name">Water</span>
              </label>
            </div>
            <div class="layer-item">
              <label class="layer-toggle">
                <input type="checkbox" bind:checked={showLandmarks} on:change={() => toggleLandmarks(showLandmarks)}>
                <span class="layer-name">Landmarks</span>
              </label>
            </div>
            <div class="layer-item">
              <label class="layer-toggle">
                <input type="checkbox" bind:checked={showLanduse} on:change={() => toggleLanduse(showLanduse)}>
