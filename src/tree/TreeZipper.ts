import type { Node, TreeZipper, Context, TreeRef } from '../transition/core';
...
// Helper functions for working with TreeRefs
export function getContributors(node: Node): TreeRef[] {
	return Array.from(node.contributors);
}

export function addContributor(node: Node, ref: TreeRef): Node {
	return {
		...node,
		contributors: new Set([...node.contributors, ref])
	};
}

export function removeContributor(node: Node, ref: TreeRef): Node {
	const newContributors = new Set(node.contributors);
	newContributors.delete(ref);
	return {
		...node,
