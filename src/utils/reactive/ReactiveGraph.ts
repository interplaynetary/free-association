...
import { createGunStore, createCollectionStore, type GunStore, type CollectionItem } from "../svelte/reactiveStores";
...
export class StoreRegistry {
  ...
  public registerDerivedStore<T>(
    ...
    if (this.wouldCreateDerivationCycle(key, dependencies)) {
      ...
      return { subscribe: (fn) => { fn(null as any); return () => {}; } };
    }
...
export class ReactiveGraph {
  ...
  public getNodeStore<T>(path: string[], defaultValue?: T): GunStore<T> {
    this.logDebug(`Getting node store for path: ${path.join('/')}`, { defaultValue, transient: this.isTransient });
  ...
  public getCollectionStore<T extends CollectionItem>(
    ...
    this.logDebug(`Getting collection store for path: ${path.join('/')}`, { hasSortFn: !!sortFn, transient: this.isTransient });
  ...
  private async _traversePath<T = any>(
    ...
    for (const segment of pathToTraverse) {
      ...
      if (!guard(currentDepth, currentPath, data)) {
        throw new Error(`Traversal stopped due to depth limit or cycle at ${currentPath.join('/')}`);
      }
  ...
  public traversePathReactive<T = any>(
    ...
    return derived(pathToTraverseStore, ($pathToTraverse, set) => {
  ...
  public async addNode<T extends CollectionItem>(
    ...
    return withErrorHandling(
      async () => {
        this.logDebug(`Adding node to collection`, { path: [...path, collectionKey], id, nodeData, transient: this.isTransient });
  ...
  public async removeNode(
    ...
    await withErrorHandling(
      async () => {
        this.logDebug(`Removing node from collection`, { path: [...path, collectionKey], nodeId, transient: this.isTransient });
   ...
   * @returns This instance for chaining
   */
  public async updateNodeProperty<T>(
    path: string[],
    property: string,
    value: T
  ): Promise<this> {
    ...
    await withErrorHandling(
      async () => {
        this.logDebug(`Updating node property`, { path, property, value, transient: this.isTransient });
  ...
  public createDerivedStore<T>(
    ...
    this.logDebug(`Creating new derived store`, { storeKey, dependencies: storeKeys });
  ...
  public getNodeProperties<T>(
    ...
    for (const prop of properties) {
      ...
      result[prop as keyof T] = derived(
        nodeStore,
        $data => $data && typeof $data === 'object' && 
          prop in ($data as object) ? 
          ($data as any)[prop] : 
          undefined
      );
    }
  ...
  public getNodePropertyStore<T>(
    ...
    this.logDebug(`Getting node property store`, { nodePath, property, defaultValue });
    
    const nodeStore = this.getNodeStore<Record<string, any>>(nodePath);
    return derived(
      nodeStore,
      $data => $data && typeof $data === 'object' && property in $data ? 
        $data[property] as T : 
        defaultValue
    );
  }
   ...
   */
  public compose(...traversals: Array<(path: string[]) => Promise<string[]>>): (startPath: string[]) => Promise<string[]> {
    return async (startPath: string[]): Promise<string[]> => {
      this.logDebug(`Executing composed traversal`, { startPath, numTraversals: traversals.length });
 ...
 */
export function createReactiveGraph(options?: TraversalOptions, transient: boolean = false): ReactiveGraph {
