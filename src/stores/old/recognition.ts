 ...
 */
function getChildPropertyStore<T>(parentPath: string[], childId: string, property: string, defaultValue?: T): Readable<T> {
...
export function createRecognitionStores(path: string[]): RecognitionStores {
  ...
  const totalChildPointsStore = aggregateCollection(
    childrenStore,
    (children) => children.reduce((sum, [_, child]) => sum + (Number(child.points) || 0), 0)
  );
  ...
  const hasDirectContributionChildStore = asyncDerivedStore(
    ...
    async (values) => {
      ...
      for (const [childId, _] of children) {
        ...
        const isContributionStore = getChildPropertyStore<boolean>(path, childId, 'isContribution', false);
  ...
  const desireStore = derived(
    fulfilledStore, 
    $fulfilled => typeof $fulfilled === 'number' ? 1 - $fulfilled : 1
  );
...
export function createContributorStores(path: string[]): ContributorStores {
  ...
  const socialDistributionsStore = derived(
    socialDistributionsRawStore, 
    $distributions => {
  ...
  const mutualRecognition = async (otherContributorId: string): Promise<Proportion> => {
    ...
    // Convert to a map
    const otherSharesMapStore = derived(
      otherSharesStore, 
      $shares => {
        const sharesMap = new Map<string, number>();
  ...
  const calculateSocialDistribution = async (depth: number = 1): Promise<DistributionMap<string>> => {
    if (depth <= 0) {
