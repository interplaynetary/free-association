...
export interface ContributorStore extends RecognitionStore {
  ...
  updatePreference: (targetId: string, value: number) => Promise<void>;
...
function createRecognitionStoreImpl(path: string[]): RecognitionStore {
  ...
  const contributionChildrenStore = graph.createDerivedStore<NodeEntry[]>(
    ['childrenStore'],
    [childrenStore],
    ([$children]) => $children.filter(([_, child]: [string, RecNode]) => Boolean(child.isContribution)) as NodeEntry[],
  ...
  const fulfillmentStore = graph.createDerivedStore<number>(
    ...
    ([$contributionChildren, $nonContributionChildren, $manualFulfillment, $totalChildPoints]) => {
      ...
      if ($totalChildPoints > 0) {
        ...
        const contributionWeight = ($contributionChildren as [string, RecNode][]).reduce(
          (sum: number, [_, child]: [string, RecNode]) => sum + (Number(child.points) || 0), 
          0
        ) / $totalChildPoints;
  ...
  const getPathToRoot = async (): Promise<Array<{
    path: string[],
    store: RecognitionStore,
    name: string,
    id: string
 ...
 */
function createContributorStoreImpl(
  path: string[]
): ContributorStore {
  ...
  const calculateSocialDistribution = async (depth: number = 2): Promise<DistributionMap<string>> => {
    // Check cache first
