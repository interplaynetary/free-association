...
export const POST: RequestHandler = async ({ request, fetch }) => {
	try {
		...
		if (!aiResponse.ok) {
			...
			error(aiResponse.status, 'Quest generation failed: ' + (errorData.message || 'Unknown error'));
		}
		...
		if (aiResult.choices?.[0]?.message?.content) {
			...
			try {
				...
				if (jsonMatch) {
					...
					error(500, 'AI did not return valid quest JSON array');
				}
			} catch (err: any) {
				console.error('[QUEST-API] Failed to parse quest JSON:', err);
				error(500, 'Failed to parse quest response: ' + err.message);
			}
		} else {
			error(500, 'AI response missing content');
		}

		if (!Array.isArray(quests) || quests.length === 0) {
			error(500, 'AI did not generate any valid quests');
		}
		...
		if (validatedQuests.length === 0) {
			error(500, 'No valid quests were generated');
		}
		...
		error(500, 'Quest generation failed: ' + err.message);
	}
};
