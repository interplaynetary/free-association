...
export const GET: RequestHandler = async ({ params }) => {
  ...
  if (model !== 'openrouter') {
    error(400, 'Only "openrouter" pool is supported. Use OpenRouter model names (e.g., anthropic/claude-3-opus)');
  }

  const keyInfo = getHealthyKey();

  if (!keyInfo) {
    error(503, 'No healthy OpenRouter keys available');
  }
...
export const POST: RequestHandler = async ({ params, request }) => {
  ...
  if (model !== 'openrouter') {
    error(400, 'Only openrouter pool is supported');
  }
