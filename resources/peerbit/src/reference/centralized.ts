...
function enterChild(childId: string, zipper: TreeZipper): TreeZipper | undefined {
  const child = zipper.zipperCurrent.nodeChildren.get(childId);
  if (!child) return undefined;
  
  const siblings = new Map(zipper.zipperCurrent.nodeChildren);
  siblings.delete(childId);
  
  const newCtx: Ctx = {
    ctxParent: zipper.zipperCurrent,
    ctxSiblings: siblings,
    ctxAncestors: zipper.zipperContext ? [zipper.zipperContext, ...zipper.zipperContext.ctxAncestors] : []
  };
...
function exitToParent(zipper: TreeZipper): TreeZipper | undefined {
  ...
  newChildren.set(zipper.zipperCurrent.nodeId, zipper.zipperCurrent);
  
  // Create a new parent with the updated children
  const newParent = { ...ctx.ctxParent, nodeChildren: newChildren };
  
  // Handle ancestors
  const newContext = ctx.ctxAncestors.length > 0 
    ? ctx.ctxAncestors[0] 
    : undefined;
...
function children(zipper: TreeZipper): TreeZipper[] {
  return Array.from(zipper.zipperCurrent.nodeChildren.keys())
...
function addToForest(forest: Forest, zipper: TreeZipper): Forest {
  ...
  newForest.set(zipper.zipperCurrent.nodeId, zipper);
  return newForest;
}
...
function createRootNode(id: string, name: string, manual?: number): RootNode {
  return {
    ...
    nodeManualFulfillment: manual !== undefined ? Math.max(0, Math.min(1, manual)) : undefined,
    nodeCapacities: new Map(),
    nodeCapacityShares: new Map(),
    nodeSOGFMap: undefined,
    nodeProviderSharesMap: new Map()
  };
}

function createNonRootNode(id: string, name: string, pts: number, contribs: string[], manual?: number): NonRootNode {
  return {
    type: "NonRootNode",
    nodeId: id,
    nodeName: name,
...
function addChild(name: string, pts: number, contribs: string[], manual: number | undefined, zipper: TreeZipper): TreeZipper | undefined {
  const current = zipper.zipperCurrent;
  
  // Don't allow adding children to nodes with contributors
  if (current.type === "NonRootNode" && current.nodeContributors.size > 0) {
    return undefined;
  }
  ...
  if (current.type === "RootNode") {
    const newChildren = new Map(current.nodeChildren);
...
function totalChildPoints(zipper: TreeZipper): number {
  return children(zipper)
    ...
    .reduce((sum, child) => sum + (child.zipperCurrent as NonRootNode).nodePoints, 0);
}
...
function weight(zipper: TreeZipper): number {
  ...
  const nodeContribution = total === 0 ? 0 : current.nodePoints / total;
...
function isContribution(zipper: TreeZipper): boolean {
  ...
  return current.nodeContributors.size > 0 && zipper.zipperContext !== undefined;
}
...
function fulfilled(zipper: TreeZipper): number {
  ...
  const hasManualFulfillment = current.nodeManualFulfillment !== undefined;
  ...
  const getManualValue = current.nodeManualFulfillment ?? 0.0;
...
function shareOfGeneralFulfillment(ci: Forest, target: TreeZipper, contributor: TreeZipper): number {
  ...
  const contribId = rootContributor.zipperCurrent.nodeId;
  ...
  const weightedContributions = contributingNodes.map(node => {
    ...
    const contributorCount = node.zipperCurrent.type === "RootNode" 
      ? 1 // Should never happen but default to 1
      : node.zipperCurrent.nodeContributors.size;
...
function findHighestMutualPath(ci: Forest, root: TreeZipper, target: TreeZipper): NavigationPath | undefined {
...
function getHighMutualNodes(ci: Forest, z: TreeZipper, threshold: number): TreeZipper[] {
  return [z, ...descendants(z)].filter(node => 
    mutualFulfillment(ci, z, node) > threshold
  );
}
...
function providerShares(ci: Forest, provider: TreeZipper, depth: number): ShareMap {
  ...
  const cachedMap = current.nodeProviderSharesMap.get(depth);
  if (cachedMap) {
    return cachedMap;
  }
  ...
  function directContributorShares(): ShareMap {
    ...
    for (const node of allContributingNodes) {
      if (node.zipperCurrent.type === "NonRootNode") {
        for (const cid of node.zipperCurrent.nodeContributors) {
    ...
    const contributorMutualFulfillments = validContributors.map(c => {
      const mf = mutualFulfillment(ci, provider, c);
      return [c.zipperCurrent.nodeId, mf] as [string, number];
    }).filter(([_, mf]) => mf > 0);
...
function getPersonalCapacityShare(ci: Forest, person: TreeZipper, capacity: Capacity): number {
  ...
  const capacityOwners = Array.from(ci.values())
    .filter(owner => {
      const rootNode = getRoot(owner).zipperCurrent;
      if (rootNode.type !== "RootNode") return false;
      return rootNode.nodeCapacities.has(capacity.capacityId);
    });
...
function computeQuantityShare(cap: Capacity, percentage: number): number {
  ...
  const percentConstrained = 
    percentage > maxPercent 
      ? Math.round(cap.quantity * maxPercent) 
      : rawQuantity;
...
function addCapacity(cap: Capacity, zipper: TreeZipper): TreeZipper {
  ...
  const current = zipper.zipperCurrent;
  const newCapacities = new Map(current.nodeCapacities);
...
function addCapacityShare(shareId: string, share: CapacityShare, zipper: TreeZipper): TreeZipper {
  ...
  const current = zipper.zipperCurrent;
  const newShares = new Map(current.nodeCapacityShares);
...
function updateComputedQuantities(zipper: TreeZipper): TreeZipper {
  ...
  for (const [key, share] of current.nodeCapacityShares.entries()) {
...
function getNodeProviderSharesMap(zipper: TreeZipper): Map<number, ShareMap> {
  const root = getRoot(zipper).zipperCurrent;
  return root.type === "RootNode" ? root.nodeProviderSharesMap : new Map();
}
...
function exampleForest(): [Forest, Forest] {
  ...
  const aliceWithChild = addChild("alice_child", 30, ["bob", "charlie"], undefined, aliceWithCapacity);
  ...
  if (charlieChildZ) {
    const charlieGrandchildZ = addChild("charlie_grandchild", 25, ["alice", "bob"], undefined, charlieChildZ);
...
function main() {
  ...
  if (aliceRoom) {
    ...
    const remainingRecipients = Array.from(aliceShares.entries())
      .filter(([id, _]) => id !== "bob" && id !== "charlie");
    ...
    console.log(`  Bob's share of Alice's room: ${receiverShareFrom(ci, bob, alice, aliceRoom, 2) * 100}%`);
    console.log(`  Charlie's share of Alice's room: ${receiverShareFrom(ci, charlie, alice, aliceRoom, 2) * 100}%`);
    
    // Show computed quantities
    console.log("\nComputed quantities for capacity shares:");
    const bobRoomQty = Math.round(aliceRoom.quantity * receiverShareFrom(ci, bob, alice, aliceRoom, 2));
    const charlieRoomQty = Math.round(aliceRoom.quantity * receiverShareFrom(ci, charlie, alice, aliceRoom, 2));
...
export {
  // Core types
  type Node, type RootNode, type NonRootNode, type TreeZipper, type Forest, type NavigationPath,
  type LocationType, type RecurrenceUnit, type RecurrenceEnd, type ShareMap,
  type Capacity, type CapacityShare, type CapacityInventory, type CapacityShares,
  
  // Core navigation and tree functions
  enterChild, exitToParent, goToRoot, modifyNode, children, descendants,
  createRootNode, createNonRootNode, addChild, addContributors, addToForest,
  
  // Calculation functions
  weight, shareOfParent, fulfilled, desire, mutualFulfillment,
  shareOfGeneralFulfillment, sharesOfGeneralFulfillmentMap, providerShares,
  
  // Capacity functions
  computeQuantityShare, createCapacityShare, addCapacity, addCapacityShare,
  receiverShareFrom, getPersonalCapacityShare, updateComputedQuantities,
