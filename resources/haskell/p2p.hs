...
-- Legacy forest type for backward compatibility
...
-- Helper to create a root node with string contributor IDs (for backward compatibility)
createRootNodeWithStringContribs :: String -> String -> Points -> [String] -> Maybe Float -> Player -> Node
createRootNodeWithStringContribs id name pts contribIds manual player =
  let contribRefs = map (createTreeRef (playerId player)) contribIds
   in createRootNode id name pts contribRefs manual
...
-- Helper to add a child with string contributor IDs (for backward compatibility)
addChildWithStringContribs :: String -> Points -> [String] -> Maybe Float -> Player -> TreeZipper -> TreeZipper
addChildWithStringContribs name pts contribIds manual player z =
  let contribRefs = map (createTreeRef (playerId player)) contribIds
   in addChild name pts contribRefs manual z
...
-- Helper function to convert a TreeRef to a root node reference
treeRefToRootRef :: TreeRef -> TreeRef
treeRefToRootRef (TreeRef pid _) = TreeRef pid pid

-- Fully resolve a TreeRef to a TreeZipper through the network if necessary
resolveTreeRef :: Network m => Player -> TreeRef -> m (Maybe TreeZipper)
resolveTreeRef = resolveNode  -- Use our new generic implementation

-- Calculate fulfillment with network support
fulfilledNetwork :: Network m => Player -> TreeRef -> m Float
fulfilledNetwork player ref = do
  maybeZipper <- resolveNode player ref
  case maybeZipper of
    Just zipper -> return $ fulfilled player zipper
    Nothing -> return 0
...
-- Legacy function for merging contributors
mergeContributors :: [Forest] -> Forest
mergeContributors = foldl' (Map.unionWith (\_ new -> new)) Map.empty

-- Check if node is a contribution node
isContributionRefNetwork :: Network m => Player -> TreeRef -> m Bool
isContributionRefNetwork player ref = do
  maybeZipper <- resolveNode player ref
  case maybeZipper of
    Just zipper -> return $ isContribution player zipper
    Nothing -> return False

-- Modified to work with TreeRef instead of String
getContributorNode :: Network m => Player -> TreeRef -> m (Maybe TreeZipper)
getContributorNode = resolveNode  -- Use our new generic implementation
